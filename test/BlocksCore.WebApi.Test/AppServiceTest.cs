...
namespace BlocksCore.WebApi.Test
{
    public class AppServiceTest
    {
        ...
        public async void TestMethodNotFound()
        {
            ...
            var result = await client.GetAsync("/WebApiTestModule/Normal/SpecialMethod");
            var resultContent = await result.Content.ReadAsStringAsync();
            Assert.Equal(System.Net.HttpStatusCode.NotFound, result.StatusCode);

            result = await client.PostAsync("/WebApiTestModule/Normal/GetMethod",null);
        ...
        {
            var client = _factory.CreateClient();
            var httpRequest = new HttpRequestMessage(HttpMethod.Get, "/WebApiTestModule/Normal/DefaultMethod") {
            ...
            result = await client.PostAsync("/WebApiTestModule/Normal/DefaultMethod", new StringContent("{}", Encoding.UTF8, "application/json"));
            Assert.Equal(System.Net.HttpStatusCode.OK, result.StatusCode);

            result = await client.PutAsync("/WebApiTestModule/Normal/DefaultMethod",   new StringContent("{}", Encoding.UTF8, "application/json"));
            Assert.Equal(System.Net.HttpStatusCode.OK, result.StatusCode);
        ...
        private async Task TestSendSimpleData(HttpMethod httpMethod, string methodName = null)
        {
            ...
            var request = new HttpRequestMessage(httpMethod, $"/WebApiTestModule/Normal/{methodName}Method?{inputParams}")
            {
        ...
        private async Task TestSendDataAndReturn(string httpMethod, string methodName = null)
        {
            ...
            var request = new HttpRequestMessage(new HttpMethod(httpMethod), $"/WebApiTestModule/Normal/{methodName}Method")
            {
                Content = new StringContent(inputString, Encoding.UTF8, "application/json")
            };
